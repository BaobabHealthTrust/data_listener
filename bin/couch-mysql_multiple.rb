...
couch_mysql_path = DIR.to_s + "/config/multiple_connection.yml"
db_settings = YAML.load_file(couch_mysql_path)

db_map_path = DIR.to_s + "/config/db_mapping_multiple.yml"
db_maps = YAML.load_file(db_map_path)

connections =  db_settings.keys

connections.each do |connection|

	connection_settings = db_settings[connection]
	couch_db_settings = connection_settings["couchdb"]
	...
	mysql_db_settings = connection_settings["mysql"]
	...
	changes "http://#{couch_username}:#{couch_password}@#{couch_host}:#{couch_port}/#{couch_db}" do
	  ...
	  database "#{mysql_adapter}://#{mysql_username}:#{mysql_password}@#{mysql_host}:#{mysql_port}/#{mysql_db}"
	  	 ...
	  	 map_keys = db_maps[connection].keys
	  	 ...
	  	 map_keys.each do |map|
	  	 	  ...
	  	 	  couch_db_fields = db_maps[connection][map].keys
			  document 'type' => document_type do
				    eval("table :#{db_table}") do
				    	
				    	couch_db_fields.each do |field|
				    		eval("column :#{field},:#{db_maps[connection][field]}")
				    	end
				    end
			  end
		 end
